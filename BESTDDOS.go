...
func main() {
	...
	go func() {
		fmt.Println("-- CODER DDOS --\n          begin to attack!\n\n")
      	        fmt.Println("-- vision : 1.3.1--\n           active attack!\n\n")
          	fmt.Println("-- phamchien.go --\n           Github: Aziz77484!\n\n")
		...
		for {
			...
			if sent%10 == 0 {
				fmt.Printf("\r%6d DOS%-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
			}
			switch <-ss {
			case callExitOnErr:
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d HITS %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
